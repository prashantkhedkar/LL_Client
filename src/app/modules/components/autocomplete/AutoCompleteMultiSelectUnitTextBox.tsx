...
function Tag(props: TagProps) {
  ...
  return (
    <div {...other}>
      <span>{label}</span>
      {!props.display && <CloseIcon onClick={onDelete} />}
    </div>
  );
}
...
const StyledTag = styled(Tag)<TagProps>(
  ...
  &:focus {
    border-color: ${theme.palette.mode === "dark" ? "#177ddc" : "#40a9ff"};
    background-color: ${
      theme.palette.mode === "dark" ? "#003b57" : "var(--primary-10)"
    };
  }
...
const Listbox = styled("ul")(
  ...
  & li[aria-selected='true'] {
    background-color: ${
      theme.palette.mode === "dark" ? "#2b2b2b" : "var(--primary-10)"
    };
    font-weight: 600;
    font: var(--text-regular-2) !important;
    & svg {
...
export default function AutoCompleteMultiSelectUnitTextBox({
  ...
  const {
    ...
    getOptionLabel: (option) => {
      // Concatenate or format multiple labels into a single string
